...
def skew_3d(omega):
    ...
    # print(omega[2])
    omega_hat[0,1] = -1 * omega[2]
    omega_hat[1,0] = omega[2]
    
    omega_hat[0,2] = omega[1]
    omega_hat[2,0] = -1 * omega[1]
    
    omega_hat[1,2] = -1 * omega[0]
...
def rotation_3d(omega, theta):
    ...
    # print(omega)
    w_hat = skew_3d(omega)
    rot_mat = i + ( w_hat * np.sin(w_norm * theta)/w_norm ) + ( w_hat.dot(w_hat)*(1 - np.cos(w_norm * theta))/(w_norm**2) )
    return rot_mat
...
def homog_3d(xi, theta):
    ...
    v = [xi[0], xi[1], xi[2]]
    # v4 = [xi[0], xi[1], xi[2], 0]
    rot_mat = rotation_3d(w, theta)
    w_hat = skew_3d([xi[3], xi[4], xi[5]])
    
    for i in range(3):
        for j in range(3):
            e_xi[i,j] = rot_mat[i,j]
    
    w_norm = np.linalg.norm(w)
    #w_transpose = np.array([w]).T
    # wcrossv = (w_hat.dot(np.array(v)))
    # dotterm = np.array(w) * np.dot(w,v)*theta
    # term1 = (np.eye(3)-rot_mat).dot(w_hat.dot(np.array(v)))
    # print("term1\n", term1, '\n\ndot term\n', dotterm)
    p = (np.eye(3)-rot_mat).dot(w_hat.dot(np.array(v))) + np.array(w) * np.dot(w,v)*theta #np.matmul(w_transpose,np.array([w])).dot()
    p = p/w_norm**2
    for k in range(3):
            e_xi[k,3] = p[k]
...
def prod_exp(xi:np.ndarray, theta):
    """
    ...
    for i in range(len(theta)):
        # print(xi)
        this_xi = [xi[0,i],xi[1,i],xi[2,i],xi[3,i],xi[4,i],xi[5,i]]
        g = np.matmul(g, homog_3d(this_xi, theta[i]))
